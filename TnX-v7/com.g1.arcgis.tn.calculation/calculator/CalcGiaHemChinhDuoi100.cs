...
{
    public class CalcGiaHemChinhDuoi100:Calculator,ICalculator
    {
        private string vitri = "";
        private int _isDothi = 1;
        public CalcGiaHemChinhDuoi100()
            : base()
        {
            ...
            if (this._inputParams.MA_XA != "-1")
            {
                ...
                while ((xaId = xaIds.Next()) != -1)
                {
                    ...
                    while ((duongId = eIds.Next()) != -1)
                    {
                        ...
                        #region log---
                        evt.Log = string.Format("\n----Kiểm tra, clip hẻm theo đường buffer {0}m, lưu vào bảng {1} ...", _currentConfig.DKhoangCach50mMatTien, hemClip100mCoSde);
                        onCalculating(evt);
                        #endregion
                        ...
                        _clipTool.ClipByLayerFileInsideSde(hemChinhFeatureLayer, hemlyr, duongBuff100FeatureLayer, duonglyr, hemClip100mNoSde);
                        try
                        {
                        ...
                        try
                        {
                            while ((hemClipId = hemChinhIds.Next()) != -1)
                            {
                                ...
                                hemChinhFt = hemFeatureClass.GetFeature(hemClipId);
                                //hemClipId = hemClipFt.OID;
                                ...
                                try
                                {
                                    ...
                                    if (gdhChinhFt != null)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            IFeature hemClipFt = null;
                                            try
                                            {
                                                hemClipFt = hemClipFcur.NextFeature();
                                                if (hemClipFt == null)
                                                {
                                                    continue;
                                                }
                                            ...
                                            wspEdit.StartEditOperation();
                                            object copiedId = copyTool.Copy(hemClipFt, gdhChinhFeatureClass);
                                            try
                                            {
                                                wspEdit.StopEditOperation();
                                                wspEdit.StopEditing(true);
                                    ...
                                    else
                                    {
                                        ...
                                        //MessageBox.Show(string.Format("line 1364 CalcGiaHemChinh heso={0},mahem={1},maduong={2}",_fcName.FC_GIA_DAT_HEM.GetIndex(_fcName.FC_GIA_DAT_HEM.HE_SO),_fcName.FC_GIA_DAT_HEM.GetIndex(_fcName.FC_GIA_DAT_HEM.MA_HEM),_fcName.FC_GIA_DAT_HEM.GetIndex(_fcName.FC_GIA_DAT_HEM.MA_DUONG)));
                                        sdeTblGdhEditor.CacheData(copiedId, rowTgdNnHandleUpdate, pairColValTgd, EnumTypeOfEdit.UPDATE);
                                        rowTgdNnHandleUpdate++;
                                        pairColValTgd.Clear();
                                        newId.Add(copiedId);
                        ...
                        //finally { Marshal.ReleaseComObject(hemClipCur); }
