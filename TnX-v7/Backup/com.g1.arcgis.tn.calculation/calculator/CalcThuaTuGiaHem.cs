...
{
    public class CalcThuaTuGiaHem : Calculator, ICalculator
    {
                ...
                while ((xaId = xaIds.Next()) != -1)
                {
                    ...
                    while ((duongId = eIds.Next()) != -1)
                    {
                        ...
                        qrf.WhereClause = string.Format("({0}='{1}' or {2} is null) and {3}='{4}'",
                                        ...
                                        _fcName.FC_GIA_DAT_HEM.MA_DUONG, maduong);
                        ...
                        while ((gdhChinhId = gdhChinhIds.Next()) != -1)
                        {
                            ...
                            int hesoHem = 3;
                            result = int.TryParse(gdhChinhFt.get_Value(gdhChinhFt.Fields.FindField(_fcName.FC_GIA_DAT_HEM.HE_SO)).ToString(), out hesoHem);
                            if (!result)
                            {
                                hesoHem = 3;
                            }
                            object giahem = gdhChinhFt.get_Value(gdhChinhFt.Fields.FindField(_fcName.FC_GIA_DAT_HEM.GIA_DAT));
                            #region lay thong tin hem
                            object mahem = gdhChinhFt.get_Value(gdhChinhFt.Fields.FindField(_fcName.FC_GIA_DAT_HEM.MA_HEM));
                            ...
                            qrf.WhereClause = string.Format("{0}='{1}'", _fcName.FC_GIA_DAT_HEM.OID, gdhChinhId);
                            ...
                            while ((thuaId = thuaIds.Next()) != -1)
                            {
                                ...
                                int hesoVitri = CalcTool.GetHeSoViTriThuaHem(dorong, hesoHem, loaidat,false);
                                if (hesoVitri == -1)
                                {
                                    continue;
                                }
