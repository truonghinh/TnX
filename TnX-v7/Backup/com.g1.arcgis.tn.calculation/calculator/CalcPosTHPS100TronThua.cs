...
{
    public class CalcPosTHPS100TronThua : Calculator, ICalculator
    {
        //!!!!tim cum tu [thaydoi] de biet cho thay doi giua cac may tinh
        //cum tu nay duoc dat truoc moi region
        private string vitri = "trong hẻm phụ, sâu < 100m";
        private int _isDothi = 1;
        public CalcPosTHPS100TronThua()
            : base()
        {
        ...
        protected override void calculate()
        {
            ...
            #region log---
            evt.Log = string.Format("\n----Lấy các quy tắc tìm vị trí thửa từ bảng {0}, ứng với hệ số {1} ...", DataNameTemplate.He_So_K, TnHeSoK.DatOHemChinhR6mS100mTronThua);
            onCalculating(evt);
            #endregion

            qrf.WhereClause = string.Format("{0}='{1}'", "hesovitri", TnHeSoK.DatOHemChinhR6mS100mTronThua);
            ...
            if (this._inputParams.MA_XA != "-1")
            {
                ...
                while ((xaId = xaIds.Next()) != -1)
                {
                    ...
                    while ((duongId = eIds.Next()) != -1)
                    {
                        ...
                        ISelectionSet hemSelectionSet = hemFeatureClass.Select(qrf, esriSelectionType.esriSelectionTypeHybrid, esriSelectionOption.esriSelectionOptionNormal, sdeConn.Workspace);
                        hemChinhFeatureSelection = (IFeatureSelection)hemChinhFeatureLayer;
                        hemChinhFeatureSelection.SelectionSet = hemSelectionSet;
                        #endregion
                        //====================
                        ...
                        while ((hemId = eIdsHem.Next()) != -1)
                        {
                            ...
                            ftHem = hemFeatureClass.GetFeature(hemId);
                            ...
                            hemSelectionSet = hemFeatureClass.Select(qrf, esriSelectionType.esriSelectionTypeHybrid, esriSelectionOption.esriSelectionOptionOnlyOne, sdeConn.Workspace);
                            hemChinhFeatureSelection = (IFeatureSelection)hemChinhFeatureLayer;
                            hemChinhFeatureSelection.SelectionSet = hemSelectionSet;
                            #endregion
                            //====================
                            ...
                            eval.HemChinhLayer = hemChinhFeatureLayer;
                            eval.EvaluateQuery();
                            //thuaFeatureSelection = (IFeatureSelection)thuaFeatureLayer;
            ...
            else
            {
                ...
                while ((duongId = eIds.Next()) != -1)
                {
                    ...
                    #region vong lap xet tung hem chinh
                    //[thaydoi] - co the them bien chay
                    //===============================
                    #region khoi dau
                    IEnumIDs eIdsHemChinh = hemChinhSelectionSet.IDs;
                    int hemChinhId;
                    IFeature ftHemChinh;
                    int iHemChinh = 0;
                    evt.Reset();
                    evt.ProgressingPart1Total = hemChinhSelectionSet.Count;
                    int progressingPart1Count = 1;
                    onCalculating(evt);
                    #endregion

                    while ((hemChinhId = eIdsHemChinh.Next()) != -1)
                    {
                        //[kodoi]
                        //======================
                        #region log-----
                        ...
                        ftHemChinh = hemFeatureClass.GetFeature(hemChinhId);
                        object mahem = ftHemChinh.get_Value(ftHemChinh.Fields.FindField(_fcName.FC_HEM.MA_HEM));
                        object tenhem = ftHemChinh.get_Value(ftHemChinh.Fields.FindField(_fcName.FC_HEM.TEN_HEM));
                        double dorong;
                        result = double.TryParse(ftHemChinh.get_Value(ftHemChinh.Fields.FindField(_fcName.FC_HEM.DO_RONG)).ToString(),out dorong);
                        if (!result)
                        {
                            dorong = 0;
                        }
                        ...
                        //====================
                        #region chon hem dang xet
                        qrf.WhereClause = string.Format("{0}='{1}'", _fcName.FC_HEM.OID, hemChinhId);
                        hemChinhSelectionSet = hemFeatureClass.Select(qrf, esriSelectionType.esriSelectionTypeHybrid, esriSelectionOption.esriSelectionOptionOnlyOne, sdeConn.Workspace);
                        hemChinhFeatureSelection = (IFeatureSelection)hemChinhFeatureLayer;
                        hemChinhFeatureSelection.SelectionSet = hemChinhSelectionSet;
                        #endregion
                        //====================

                    //    #region vong lap xet tung hem phu
                    ////[thaydoi] - co the them bien chay
                    ////===============================
                    //#region khoi dau
                    //IEnumIDs eIdsHemPhu = hemChinhSelectionSet.IDs;
                    //int hemPhuId;
                    //IFeature ftHemPhu;
                    //int iHemPhu = 0;
                    ////evt.Reset();
                    ...
                    //    //====================
                        ...
                        if (thuaSelectionSet.Count == 0)
                        {
                            ...
                            #region report progressing hem
                            if (iHemChinh < hemChinhSelectionSet.Count)
                            {
                                decimal i = (decimal)iHemChinh % (decimal)hemChinhSelectionSet.Count * 100;
                                int i1 = Convert.ToInt32(i);
                                this._bgwCalculating.ReportProgress(i1);
                                //MessageBox.Show("log 009");
                            }
