...
{
    public class CalcGiaHemChinh100_200 : Calculator, ICalculator
    {
        private string vitri = "";
        private int _isDothi = 1;
        public CalcGiaHemChinh100_200()
            : base()
        {
        ...
        protected override void calculate()
        {
            ...
            string hemClip200mNoSde = FilterSdeLayerName.GetActualName(DataNameTemplate.Hem_Clip_);
            hemClip200mNoSde += "200";
            //string duongBuffer50mCoSde = string.Format("{0}{1}", DataNameTemplate.Duong_Buffer_, 50);
            string hemClip200mCoSde = string.Format("{0}{1}", DataNameTemplate.Hem_Clip_, 200);
            ...
            {
                _bufferTool.BufferInsideSde(duongFeatureClass.AliasName, duongBuffer100mNoSde, 100);
            }
            ...
            if (this._inputParams.MA_XA != "-1")
            {
                ...
                while ((xaId = xaIds.Next()) != -1)
                {
                    ...
                    while ((duongId = eIds.Next()) != -1)
                    {
                        ...
                        #region log---
                        evt.Log = string.Format("\n----Kiểm tra, xóa bảng {0} ...", hemClip200mCoSde);
                        onCalculating(evt);
                        #endregion

                        if (_dataManager.LayerExist(hemClip200mNoSde))
                        {
                            ...
                            ((IFeatureClassManager)_dataManager).DeleteFcInSde(hemClip200mCoSde);
                        ...
                        #region chon duong buffer 200 dang xet
                        qrf.WhereClause = string.Format("{0}='{1}'", _fcName.FC_DUONG.OID, duongId);
                        ...
                        #region log---
                        evt.Log = string.Format("\n----Kiểm tra, clip hẻm theo đường buffer {0}m, lưu vào bảng {1} ...", 200, hemClip200mCoSde);
                        onCalculating(evt);
                        #endregion

                        string hemlyr = string.Format("{0}/{1}", _sysTempPath.TempPath, hemClip200mNoSde);
                        string duonglyr = string.Format("{0}/{1}", _sysTempPath.TempPath, duongBuffer200mNoSde);
                        _clipTool.ClipByLayerFileInsideSde(hemChinhFeatureLayer, hemlyr, duongBuff200FeatureLayer, duonglyr, hemClip200mNoSde);
                        try
                        {
                            hemClipFc = fw.OpenFeatureClass(hemClip200mCoSde);
                        ...
                        string duongErlyr = string.Format("{0}/{1}", _sysTempPath.TempPath, duongBuffer100mNoSde);
                        _eraseTool.EraseByLayerFileInsideSde(hemClipFlayer, hemlyr, duongBuff100FeatureLayer, duonglyr, hemErase100mNoSde);
                        #endregion
